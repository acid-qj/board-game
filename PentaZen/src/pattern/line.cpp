...
int Line::judge(Material m) const {
    ...
    case B2:
    case B1:
        ret = ret < 5 - piece_req_of(m) ? 0 : (int)ceil(ret / (5.0 - piece_req_of(m)));
        break;
    case F2:
        ret = ret < 2 ? 0 : (int)ceil(ret / 4.0);
        break;
    case F1:
        ret = ret < 3 ? 0 : (int)ceil(ret / 6.0);
        break;
    default:
        break;
    }
...
void generate(int len) {
    ...
    for (auto m = C6; m != MATERIAL_NUM; ++m)
        for (auto i = 0; i != (1 << len); ++i) {
            line.set(i, len);
            MatTable[line.index()][m] = line.judge(m);
        }
}
